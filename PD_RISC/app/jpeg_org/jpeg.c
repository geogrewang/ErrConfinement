...
int Fileinput(int imageData[SIZE][SIZE])
{
   ...
   //myFile_512 is the text file in which the input file is defined
    ifp = fopen("/net/home/wangz/wangz/zheng-work/CI_QoS/Arch/PD_RISC_LLVM/app/jpeg_org/lena512.txt", "r");
    if (ifp == NULL) 
    {
...
int Fileoutput(int imageExtended[SIZE][SIZE])
{
    ...
    ofp = fopen("/net/home/wangz/wangz/zheng-work/CI_QoS/Arch/PD_RISC_LLVM/app/jpeg_org/outputData_dct.txt", "w");
        if (ofp == NULL)
         {
